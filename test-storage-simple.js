...
function createTestImage() {
  ...
  // Criar imagem PNG simples (1x1 pixel)
  const pngBuffer = Buffer.from([
    ...
    0x00, 0x00, 0x03, 0x00, 0x01, 0x5d, 0x3f, 0xaf,
    0xcf, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
...
async function testUpload(imagePath) {
  ...
  try {
    ...
    form.append('image', fs.createReadStream(imagePath), 'test-image.png');
    form.append('workflow', 'catalog');

    log('info', `Enviando arquivo: test-image.png (${fileContent.length} bytes)`);
    log('info', `Endpoint: POST ${API_BASE}/api/ai/submit`);

    const apiKey = process.env.API_KEY || 'test-default-key';
